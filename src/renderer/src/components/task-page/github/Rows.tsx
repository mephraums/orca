    ...
    <div className="divide-y divide-border/40">
      {!showGitHubTaskSkeletons &&
        filteredWorkItems.map((item) => {
          ...
          const rowSelectable = prBatchSelection.prMultiSelectColumnsActive && item.type === 'pr'
          const rowSelected =
            rowSelectable && prBatchSelection.selectedBatchPrKeys.has(prSelectionKey(item))
          const activateRow = (): void =>
            rowSelectable ? prBatchSelection.handleToggleBatchPr(item) : openGitHubDetailPage(item)
          ...
          return (
            ...
            <div
              ...
              // Why: in multi-select mode the whole row is the toggle target, so it announces as a checkbox.
            ...
            >
              <div className={GITHUB_TASK_STICKY_ID_CELL_CLASS}>
                {rowSelectable ? (
                  // Why: decorative — the row carries the checkbox role, so this box only mirrors its state.
                  <Checkbox
                    checked={rowSelected}
                    tabIndex={-1}
                    aria-hidden="true"
              ...
              <div className={prBatchSelection.githubTaskStickyTitleCellClass}>
